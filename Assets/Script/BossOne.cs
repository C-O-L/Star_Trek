...
public class BossOne : MonoBehaviour
{
    ...
    public void Fire()
    {
        ...
        //使用四元数制造绕Z轴旋转15度的旋转
        Quaternion startQuaternion = Quaternion.AngleAxis(15, Vector3.forward);
        for(int j = 0; j < 24; j++)
        {
            //生成子弹
            ...
            fireDirection = startQuaternion * fireDirection;                //发射方向旋转15度，到达下一个方向
            //定时销毁子弹
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if(collision.CompareTag("Bullet"))
        {
            ...
            TakeDamage(20);                                                 //调用TakeDamage方法,血量减20
        }
        //如果碰到子弹plus
        if(collision.CompareTag("BulletPlus"))
        {
            Destroy(collision.gameObject);
            TakeDamage(50);                                                 //调用TakeDamage方法,血量减50
        }
    }
    ...
    public void TakeDamage(int loss)
    {
        ...
        if(hp == 0){
            ...
            // GameManager.instance.isEnemy = true;                                //boss1死后开始生成普通敌人
